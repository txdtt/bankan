        ...
        if (columnDialog) {
            ...
            editColumnTitle.onclick = () => {
                ...
                saveButton.onclick = () => {
                    ...
                    const columnIndex = columnModel_1.columns.findIndex(col => col._id === column._id);
                    if (column._id) {
                        (0, columnService_1.patchColumn)(column._id, inputTitle.value);
                    }
                    if (columnIndex !== -1) {
                        columnModel_1.columns.splice(columnIndex, 1, Object.assign(Object.assign({}, columnModel_1.columns[columnIndex]), { title: inputTitle.value }));
                    }
                };
...
async function submitColumn(titleInput) {
    ...
    if (titleInput && title) {
        ...
        const columnToRender = columnModel_1.columns.find(column => column.title === title);
        if (columnToRender) {
