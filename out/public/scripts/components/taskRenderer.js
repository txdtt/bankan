            ...
            editTaskTitle.onclick = () => {
                ...
                const saveTitle = async () => {
                    ...
                    if (columnElement) {
                        ...
                        const columnIndex = columnModel_1.columns.findIndex(column => column._id === columnId);
                        if (columnIndex !== -1) {
                            const taskId = taskElement.id;
                            if (taskId) {
                                await (0, taskService_1.patchTaskTitle)(columnElement.id, taskId, inputTitle.value);
                                const taskIndex = columnModel_1.columns[columnIndex].tasks.findIndex(task => task._id === taskId);
                                (0, dragAndDrop_1.removeTaskDragAndDrop)(columnModel_1.columns[columnIndex].tasks[taskIndex]);
                                (0, dragAndDrop_1.setupTaskDragAndDrop)(columnModel_1.columns[columnIndex].tasks[taskIndex]);
                            }
                        }
                    }
                };
...
async function submitTask(columnSelected, title, description) {
    ...
    try {
        ...
        const columnData = columnModel_1.columns.find(column => column._id === columnSelected);
        if (!columnData) {
