...
async function handleDragEnd(e, task) {
    ...
    if (dragging && placeholder) {
        ...
        if (columnElement && sourceColumnId) {
            ...
            const columnIndex = Column_1.columns.findIndex(column => column._id === columnElement.id);
            if (task._id) {
                ...
                if (newIndex !== -1) {
                    if (sourceColumnId === targetColumnId) {
                        if (task._id) {
                            const currentIndex = Column_1.columns[columnIndex].tasks.findIndex((t) => t._id === task._id);
                            Column_1.columns[columnIndex].tasks.splice(currentIndex, 1);
                            Column_1.columns[columnIndex].tasks.splice(newIndex, 0, { title: task.title, description: task.description, _id: task._id });
                            await (0, TaskService_1.updateTaskOrder)(targetColumnId, Column_1.columns[columnIndex].tasks);
                        }
                    }
                    else {
                        Column_1.columns[columnIndex].tasks.splice(newIndex, 0, { title: task.title, description: task.description, _id: task._id });
